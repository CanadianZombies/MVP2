...
logsearch (CHAR_DATA * ch, char *argy)
{
  ...
  char fn[50];
  int i;
  DEFINE_COMMAND ("logsearch", logsearch, POSITION_DEAD, 110, LOG_NORMAL, "Allows you to spawn a log grepping process...")
    sh = new_shell ();
...
void
pk (CHAR_DATA * ch, char *argy)
{
  SHELL *sh;
  char tmpln[500];
  char fn[50];
  int i;
  DEFINE_COMMAND ("pk", pk, POSITION_STANDING, 15, LOG_NORMAL, "Allows you to spawn a log grepping process...")
    sh = new_shell ();
  fn[0] = number_range ('a', 'z');
  fn[1] = number_range ('a', 'z');
  fn[2] = number_range ('a', 'z');
  ...
  strcpy (sh->fname, fn);
  strcpy (sh->caller, NAME (ch));
  sprintf (tmpln, "grep warpoints ../log/* > %s &", fn);
  system (tmpln);
  send_to_char ("PK search started.\n\r", ch);
  return;
}
...
finger (CHAR_DATA * ch, char *argy)
{
  ...
  char tmpln[500];
  char fn[50];
  DEFINE_COMMAND ("sysfinger", finger, POSITION_DEAD, 110, LOG_ALWAYS, "Allows you to spawn a finger process...") sh =
    new_shell ();
  fn[0] = number_range ('a', 'z');
  fn[1] = number_range ('a', 'z');
  fn[2] = number_range ('a', 'z');
...
syslag (CHAR_DATA * ch, char *argy)
{
  ...
  char tmpln[500];
  char fn[50];
  DEFINE_COMMAND ("syslag", syslag, POSITION_DEAD, 110, LOG_ALWAYS,
		  "Shows system memory reqs, etc...") sh = new_shell ();
  fn[0] = number_range ('a', 'z');
  fn[1] = number_range ('a', 'z');
  fn[2] = number_range ('a', 'z');
  fn[3] = number_range ('a', 'z');
...
#else
  sprintf (tmpln, "ps -aux | grep MpV > %s &", fn);
#endif
  system (tmpln);
