...
do_commands (CHAR_DATA * ch, char *argy)
{
  ...
  DEFINE_COMMAND ("commands", do_commands, POSITION_DEAD, 0, LOG_NORMAL,
		  "Help on commands, or a command list.") argy =
    one_argy (argy, arg);
  if (is_number (arg))
    {
      minlev = atoi (arg);
...
do_socials (CHAR_DATA * ch, char *argy)
{
  ...
  SOCIAL *s;
  int col;
  DEFINE_COMMAND ("socials", do_socials, POSITION_DEAD, 0, LOG_NORMAL,
		  "This command shows all socials currently in the database.")
    col = 0;
