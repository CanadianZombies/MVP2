...
do_spico_help (CHAR_DATA * ch, char *argy)
{
  DEFINE_COMMAND ("edit_help", do_spico_help, POSITION_DEAD, 110, LOG_ALWAYS,
		  "Remote Help Editing.") if (!ch->desc)
    return;
...
do_shell (CHAR_DATA * ch, char *argy)
{
  char arg1[500];
  char arg2[500];
  DEFINE_COMMAND ("shell", do_shell, POSITION_DEAD, 110, LOG_ALWAYS,
		  "Do not use.") if (!ch->desc)
    return;
  return;
}
