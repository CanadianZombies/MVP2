...
find_trigger (char *trigname)
{
  int i;
  SINGLE_TRIGGER *tr;
  MARK_DEBUG ("Find_trigger") for (i = 0; i < TMAX_TRIGGERS; i++)
  {
...
list_triggers (CHAR_DATA * ch, char *argy)
{
  int i;
  SINGLE_TRIGGER *tr;
  DEFINE_COMMAND ("listtriggers", list_triggers, POSITION_DEAD, 110,
      LOG_NORMAL, "Lists all triggers.") hugebuf_o[0] = '\0';
  sprintf (hugebuf_o, "%-20s %s %5s %5s %5s\n\r", "Trigger ID", "TrigType  ",
      "Room", "Mob", "Objct");
  strcat (hugebuf_o,
      "------------------------------------------------------------------------------\n\r");
  for (i = 0; i < TMAX_TRIGGERS; i++)
  {
    for (tr = trigger_list[i]; tr != NULL; tr = tr->next)
    {
      sprintf (hugebuf_o + strlen (hugebuf_o), "%-20s %s %5d %5d %5d\n\r",
          ...
          tr->attached_to_room,
          tr->attached_to_mob, tr->attached_to_obj);
    }
  }
...
script_editor (CHAR_DATA * ch, char *argy)
{
  ...
  char arg2[500];
  SINGLE_TRIGGER *tr;
  DEFINE_COMMAND ("scriptedit", script_editor, POSITION_DEAD, 110, LOG_NORMAL,
      "For Kilith's use only right now.")
  if (!ch->desc) 
    return;
  argy = one_argy (argy, arg1); argy = one_argy (argy, arg2);
  ...
  if (arg1[0] == '\0')
    return; tr = find_trigger (arg1); if (!tr)
    {
...
void show_this_trigger (CHAR_DATA * ch, char *trig)
{
  SINGLE_TRIGGER * tr;
  char buf[500]; if ((tr = find_trigger (trig)) == NULL)
  {
    send_to_char ("Trigger not found.\n\r", ch);
    return;
  }
  sprintf (buf,
      "Trigger ID: \x1B[37;1m%s\x1B[37;0m  Trigger Type: \x1B[37;1m%s\x1B[37;0m\n\r",
      tr->trigger_id, trigger_type (tr->trigger_type));
  send_to_char (buf, ch);
  sprintf (buf,
      "Room: %d  Mob: %d  Object: %d  Interruptable: %s\n\r",
      tr->attached_to_room, tr->attached_to_mob,
      tr->attached_to_obj,
      (tr->interrupted ==
       1 ? "Yes" : (tr->interrupted ==
         2 ? "Yes, and multiple!" : "No")));
  send_to_char (buf, ch);
  sprintf (buf,
      "\x1B[37;1mPlayer\x1B[37;0m only: %s  \x1B[37;1mStop\x1B[37;0ms if player leaves: %s\n\r",
      (tr->players_only == 1 ? "Yes" : "No"),
      (tr->leaves_room == 1 ? "Yes" : "No"));
  send_to_char (buf, ch);
  sprintf (buf,
      "Calls code label: \x1B[34;1m%s\x1B[37;0m\n\r",
      tr->code_label); send_to_char (buf, ch);
      sprintf (buf, "Keywords: \x1B[37;1m%s\x1B[37;0m\n\r",
          tr->keywords); send_to_char (buf, ch);
          return;
}
...
void remove_trigger_from_list (SINGLE_TRIGGER * tr)
{
  ...
  for (tt = trigger_list[tr->trigger_type]; tt != NULL;
      tt = tt->next)
  {
    if (tt->next == tr)
    {
      tt->next = tr->next; tr->next = NULL;
      return;
    }
  }
  fprintf (stderr,
      "Trigger not found to remove from trigger list!!\n");
  return;
}

void scriptedit (CHAR_DATA * ch, char *argy)
{
  SINGLE_TRIGGER * tr;
  int tmp;
  char orig_argy[500];
  char arg1[500]; if (!ch->desc) return; if (!ch->desc->pEdit)
  {
  ...
  {
    remove_trigger_from_list (tr); tr->trigger_type = tmp;
  ...
  if (!str_cmp (arg1, "code") && argy[0] != '\0')
  {
    ...
    {
      send_to_char
        ("Invalid code label.  Must be less than 10 letters.\n\r",
         ch);
      return;
    }
  ...
  if (!str_prefix ("player", arg1))
  {
    ...
    if (tr->players_only)
    {
      ...
      send_to_char
        ("Script can now be triggered by mobs as well.\n\r", ch);
    }
    else
    {
      tr->players_only = TRUE;
      send_to_char
        ("Script can only be triggered by players now.\n\r", ch);
    }
    return;
  }
  if (!str_prefix ("stop", arg1))
  {
    ...
    if (tr->leaves_room)
    {
      ...
      send_to_char
        ("Script continues if char leaves the room.\n\r", ch);
    }
    else
    {
      tr->leaves_room = TRUE;
      send_to_char
        ("Script now stops if the character leaves the room.\n\r",
         ch);
    }
    return;
  }
...
/* Finds a code label */
CODE * find_code (char *id)
{
  CODE * c; int i; for (i = 0; i < 256; i++)
  {
    for (c = code_list[i]; c != NULL; c = c->next)
    {
      if (!str_cmp (id, c->label)) return c;
    }
  }
  return NULL;
}

CODE * new_code (char *id)
{
  CODE * c;
  ...
  if (strlen (id) > 9)
    id[9] = '\0';
  strcpy (c->label, id);
  c->next = code_list[(int) c->label[0]];
  code_list[(int) c->label[0]] = c;
  c->code = &str_empty[0];
  return c;
}

void online_coding (CHAR_DATA * ch, char *argy)
{
  char arg1[500];
  CODE * cd;
  CODE * c;
  DEFINE_COMMAND ("edit", online_coding, POSITION_DEAD, 110,
      LOG_NORMAL,
      "Add/Modify online script code.") argy =
    one_argy (argy, arg1);
  if (!str_cmp (arg1, "delete") && argy[0] != '\0')
  {
    ...
    if (code_list[(int) cd->label[0]] == cd)
    {
      code_list[(int) cd->label[0]] = cd->next;
    }
    else
      for (c = code_list[(int) cd->label[0]]; c != NULL;
          c = c->next)
      {
        if (c->next == cd)
        {
          c->next = cd->next; goto duu;
        }
      }
duu:
    free_string (cd->code);
  ...
  if (!str_cmp (arg1, "create") && argy[0] != '\0')
  {
    ...
    cd = new_code (argy); string_append (ch, &cd->code);
    return;
  }
...
void list_code (CHAR_DATA * ch, char *argy)
{
  int i;
  int col = 0;
  CODE * tr;
  DEFINE_COMMAND ("listcode", list_code, POSITION_DEAD, 110,
      LOG_NORMAL,
      "Lists all code labels.") hugebuf_o[0] =
    '\0'; for (i = 0; i < 256; i++)
    {
...
void save_triggers (void)
{
  FILE * fp;
  SINGLE_TRIGGER * tr;
  int i; if ((fp = fopen ("trig.dat", "w+")) == NULL)
  {
    fprintf (stderr,
        "Error on trigger write open!!  NOT GOOD!\n");
    return;
  }
  for (i = 0; i < TMAX_TRIGGERS; i++)
  {
...
void load_triggers (void)
{
  ...
  FILE * fp;
  int ntr;
  char txt[100]; if ((fp = fopen ("trig.dat", "r")) == NULL)
  {
    return;
  }
  ...
  for (;;)
  {
    ...
    if (!str_cmp (txt, "END")) break; if (!str_cmp (txt, "ID"))
    {
      strcpy (txt, fread_string2 (fp, NULL));
      tr = new_trigger (txt); continue;
    }
    ...
    if (!str_cmp (txt, "TT"))
    {
      ...
      trigger_list[tr->trigger_type] = tr; continue;
    }

    if (!str_cmp (txt, "Keywords"))
    {
      tr->keywords = fread_string (fp, NULL); continue;
    }
    if (!str_cmp (txt, "Room"))
    {
      tr->attached_to_room = fread_number (fp); continue;
    }
    if (!str_cmp (txt, "Mob"))
    {
      tr->attached_to_mob = fread_number (fp); continue;
    }
    if (!str_cmp (txt, "Obj"))
    {
      tr->attached_to_obj = fread_number (fp); continue;
    }
    if (!str_cmp (txt, "Plo"))
    {
      tr->players_only = fread_number (fp); continue;
    }
    if (!str_cmp (txt, "Clr"))
    {
      tr->leaves_room = fread_number (fp); continue;
    }
    if (!str_cmp (txt, "Int"))
    {
      tr->interrupted = fread_number (fp); continue;
    }
    if (!str_cmp (txt, "Code"))
    {
      strcpy (tr->code_label, fread_string2 (fp, NULL)); continue;
    }
  }
...
void save_code (void)
{
  FILE * fp;
  CODE * c;
  int i; if ((fp = fopen ("code.dat", "w+")) == NULL)
  {
...
void load_code (void)
{
  CODE * c = NULL;
  char txt[1100];
  FILE * fp; if ((fp = fopen ("code.dat", "r")) == NULL)
  {
    return;
  }
  ...
  for (;;)
  {
    strcpy (txt, fread_word (fp));
    if (!str_cmp (txt, "END"))
      break; if (!str_cmp (txt, "LABEL"))
      {
        strcpy (txt, fread_string2 (fp, NULL));
        c = new_code (txt); continue;
      }
    if (!str_cmp (txt, "CODE"))
    {
      c->code = fread_string (fp, NULL); continue;
    }
  }
...
CHAR_DATA * debugger = NULL;	/* This holds who debugging should go to */
  char e_buf[500]; int executed_lines = 0;	/* Keeps track of lines executed.. traps repeating loops */
void debug (CHAR_DATA * ch, char *argy)
{
  DEFINE_COMMAND ("debug", debug, POSITION_DEAD, 110,
      LOG_NORMAL,
      "Sets all script error messages to go to you.")
    debugger = ch;
  send_to_char
    ("You now will recieve all script error messages.\n\r", ch);
  return;
}



void end_script (SCRIPT_INFO * scr)
{
  SCRIPT_INFO * ls;
  MARK_DEBUG ("End_script")
    scr->called_by->running_info = NULL; if (scr == info_list)
    {
      info_list = scr->next; scr->next = NULL;
    }
  else
    for (ls = info_list; ls != NULL; ls = ls->next)
    {
      if (ls->next == scr)
      {
        ls->next = scr->next; scr->next = NULL; break;
      }
    }
  free (scr);
  return;
...
void
e_goto_invalid_line_number (SCRIPT_INFO * scr, char *ln,
    int linenum)
{
  sprintf (e_buf,
      "Invalid goto line number '%s' in code label %s, line %d.  Script ended.\n\r",
      ln, scr->code_seg, linenum);
  if (debugger) send_to_char (e_buf, debugger);
  end_script (scr);
  return;
}

void
e_couldnt_find_label (SCRIPT_INFO * scr, char *lab,
    int linenum)
{
  sprintf (e_buf,
      "Couldn't find code label %s, line %d, in trigger %s.  Script ended.\n\r",
...
void e_bad_wait_value (SCRIPT_INFO * scr, char *cmd)
{
  sprintf (e_buf,
      "Bad wait value of '%s', line %d, code label %s.  Script ended.\n\r",
...
void e_bad_wait_type (SCRIPT_INFO * scr, char *typ)
{
  sprintf (e_buf,
      "Bad wait type '%s', line %d, code label %s.  Script ended.\n\r",
...
void e_repeating_loop (SCRIPT_INFO * scr)
{
  sprintf (e_buf,
      "Apparent repeating loop in code label %s, trigger %s.  Script ended.\n\r",
...
void e_invalid_create_obj (SCRIPT_INFO * scr, char *ln)
{
  sprintf (e_buf,
      "Invalid create object vnum '%s', code label %s line %d.  Script ended.\n\r",
...
void e_invalid_create_mob (SCRIPT_INFO * scr, char *ln)
{
  sprintf (e_buf,
      "Invalid create mob vnum '%s', code label %s line %d.  Script ended.\n\r",
      ln, scr->code_seg, scr->current_line - 1);
  if (debugger) send_to_char (e_buf, debugger);
  end_script (scr);
  return;
...
void e_bad_if (SCRIPT_INFO * scr, char *ln)
{
  sprintf (e_buf,
      "Invalid if statement '%s', code label %s line %d.  Script ended.\n\r",
      ln, scr->code_seg, scr->current_line - 1);
  if (debugger) send_to_char (e_buf, debugger);
  end_script (scr);
  return;
...
void e_bad_destroy_num (SCRIPT_INFO * scr, char *ln)
{
  sprintf (e_buf,
      "Bad destroy vnum: %s, code label %s line %d.  Script ended.\n\r",
      ln, scr->code_seg, scr->current_line - 1);
  if (debugger) send_to_char (e_buf, debugger);
  end_script (scr);
  return;
...
char *substitue_vars (SCRIPT_INFO * scr, char *ln)
{
  static char curln[8000];
  char *t;
  int pos = 0;
  curln[0] = '\0';
  MARK_DEBUG ("Substitute_vars") for (t = ln; *t != '\0'; t++)
  {
    if (*t == '@')
    {
      t++; if (*t == '\0') break; if (UPPER (*t) == 'C')
      {
        char toad[8000];
        toad[0] = '\0'; curln[pos] = '\0'; t++; if (*t == 't')
        {
          ...
          if (*t == 'r')
          {
            ...
            if (*t == '!')
            {
              sprintf (toad, "%d", scr->current->pcdata->race);
            }
            else
              if (*t == 'K')
              {
                        ...
                        if (*t == 'M')
                        {
                          ...
                          if (*t == 'H')
                          {
                            ...
                            if (*t == 'h')
                            {
                              ...
                              {
                                sprintf (toad, "%d",
                                    (IS_PLAYER (scr->current) ? scr->current->pcdata->
                                     warpoints : 0));
                              }
                              else
                                if (*t == 'g')
        ...
        strcat (curln, toad); pos += strlen (toad); continue;
      }
      if (UPPER (*t) == 'V')
      {
        char vv[500];
        curln[pos] = '\0';
        if (scr->obj
            && scr->obj->pIndexData->item_type == ITEM_CORPSE_NPC)
        {
          sprintf (vv, "%d",
              ((I_CONTAINER *) (scr->obj->more))->key_vnum);
          strcat (curln, vv); pos += strlen (vv);
        }
        else
          if (scr->mob)
          {
            ...
            strcat (curln, tmp); pos += strlen (tmp);
          }
          else
            if (scr->obj)
            {
      ...
      {		/* Random number eg... @R(1,10) */
        ...
        t++;
        if (*t != '(')
          continue; t++; for (; *t != ',' && *t != ')'; t++)
          {
            num1[tt] = *t; tt++;
          }
        num1[tt] = '\0'; t++; tt = 0; for (; *t != ')'; t++)
        {
          if (*t != ' ') num2[tt] = *t; tt++;
        }
        {
          ...
          sprintf (tmp, "%d", rn);
          strcat (curln, tmp); pos += strlen (tmp); continue;
        }
      }
      continue;
    }
...
void execute_code (SCRIPT_INFO * scr)
{
  CODE * c;
  char cur_line[5000];
  char arg1[5000];
  ...
  if (scr->called_by && scr->called_by->leaves_room &&
      ((scr->mob
        && scr->mob->in_room != scr->current->in_room)
...
execute:			/* Main execute loop */
  ...
  scr->tick_type = 0;
  scr->delay_ticks = 0;
  c = NULL; if ((c = find_code (scr->code_seg)) == NULL)
  {
  ...
  if (scr->current_line == 0)
    scr->current_line = 1; for (t = c->code; *t != '\0'; t++)
    {
      if (*t == ':') linenum++; if (linenum == scr->current_line)
      {
        t++; for (; *t != ' ' && *t != ':' && *t != '\0'; t++)
        {
        }
  ...
  one_argy (curl2, argg); ln = cur_line;
  ...
  {		/* Call <other code label> */
    char clab[500];
    ln = one_argy (ln, arg1); if ((c = find_code (ln)) == NULL)
    {
    ...
    scr->current_line = 1;
    execute_code (scr); strcpy (scr->code_seg, clab);
    return;
  }

  else
    if (!str_cmp (argg, "goto"))
    {
      int totl = 0;
      int gott = 0;
      char *tt;
      ...
      gott = atoi (ln); if (gott < 1 || gott > totl)
      {
        e_goto_invalid_line_number (scr, ln, scr->current_line - 1);
        return;
      }
      scr->current_line = gott;
    }

    else
      if (!str_cmp (argg, "wait"))
      {
        char *l; char rg1[500]; char rg2[500]; int wait; l = cur_line; l = one_argy (l, rg1); l = one_argy (l, rg2);	/* rg1 holds wait, rg2 holds number, l holds tick type */
        if ((wait = atoi (rg2)) < 1)
        {
        ...
        {
          scr->tick_type = 4; scr->delay_ticks = wait;
        }
        else
          if (!str_cmp (l, "quick"))
          {
            scr->tick_type = 1; scr->delay_ticks = wait;
          }
          else
            if (!str_prefix ("heart", l))
            {
              scr->tick_type = 2; scr->delay_ticks = wait;
            }
            else
              if (!str_cmp (l, "hour"))
              {
                scr->tick_type = 3; scr->delay_ticks = wait;
              }
              else
              {
        ...
        if (!str_cmp (cur_line, "done"))
        {
          ...
          if (!str_prefix ("create_mob", argg))
          {
            int mnum;
            MOB_PROTOTYPE * m;
            CHAR_DATA * mob;
            char *l = cur_line;
            l = one_argy (l, argg); if (!is_number (l))
            {
              e_invalid_create_mob (scr, l);
              return;
            }
            mnum = atoi (l); if ((m = get_mob_index (mnum)) == NULL)
            {
              e_invalid_create_mob (scr, l);
              return;
            }
            ...
            if (scr->obj)
            {
              if (scr->obj->carried_by != NULL)
                ...
                if (scr->obj->in_room != NULL)
                  char_to_room (mob, scr->obj->in_room);
            }
          }

          else
            if (!str_prefix ("destroy_mob", argg))
            {
              char *l = cur_line;
              l = one_argy (l, argg); if (scr->current)
              {
              ...
              if (!str_prefix ("destroy_obj", argg))
              {
                char *l = cur_line;
                OBJ_PROTOTYPE * op; l = one_argy (l, argg); if (scr->obj)
                {
                  obj_from (scr->obj); free_it (scr->obj);	/*free_it also stops ends script! */
                  return;
                }
                if (!is_number (l))
                {
                ...
                if (scr->mob)
                {
                  ...
                  bool des = FALSE;
                  for (o = scr->mob->carrying; o != NULL && !des;
                      o = o_next_content)
                  {
                    o_next_content = o->next_content; if (o->pIndexData == op)
                    {
                      obj_from (o); free_it (o); des = TRUE;
                    }
                  }
                }
                else
                  if (scr->room)
                  {
                    ...
                    bool des = FALSE;
                    if (scr->room->more)
                      for (o = scr->room->more->contents; o != NULL && !des;
                          o = o_next_content)
                      {
                        o_next_content = o->next_content; if (o->pIndexData == op)
                        {
                ...
                if (!str_prefix ("create_obj_char", argg))
                {
                  int onum;
                  OBJ_PROTOTYPE * o;
                  SINGLE_OBJECT * obj;
                  ...
                  onum = atoi (l); if ((o = get_obj_index (onum)) == NULL)
                  {
                    e_invalid_create_obj (scr, l);
                    return;
                  }
                  obj = create_object (o, 1); if (scr->current)
                  {
                  ...
                  if (!str_prefix ("create_obj", argg))
                  {
                    ...
                    if (!str_cmp (argg, "if"))
                    {
                      ...
                      bool blt = FALSE;
                      bool bne = FALSE;
                      char rightnum[50]; l = one_argy (l, argg); if (*l == '(')
                      {
                        int st; bool ne = FALSE; l++; if (*l == '!')
                        {
                          ne = TRUE; l++;
                        }
                        for (tc = l; *tc != ')'; tc++)
                        ...
                        leftnum[cci] = '\0'; for (st = 0; st < 3000; st++)
                        {
                          if (!scr->current->pcdata->script_flags[st]
                              || scr->current->pcdata->script_flags[st][0] ==
                              '\0') continue;
                          if (!str_cmp
                              (scr->current->pcdata->script_flags[st],
                               leftnum)) goto matched;
                        }
                        scr->current_line++; goto dunw; matched:
                          if (ne) scr->current_line++; dunw:
                            if (scr)
                            {
                            };
                      }
                      else
                      {
                            ...
                            tc++)
                        {
                          ...
                          if (*tc == ' ') continue; leftnum[cci] = *tc; cci++;
                        }
                        leftnum[cci] = '\0'; if (*tc == '=')
                        {
                          if (*(tc + 1) == '=') tc++; beq = TRUE;
                        ...
                        rightnum[cci] = '\0'; if (is_number (leftnum))
                        {
                          int lnm;
                          int rnm;
                          lnm = atoi (leftnum);
                          rnm = atoi (rightnum); if (beq && lnm != rnm)
                          {
                            scr->current_line++;
                          }
                          else
                      ...
                      if (!str_cmp (argg, "recho"))
                      {
                        char *l = cur_line;
                        l = one_argy (l, argg); do_recho (scr->current, l);
                      }

                      else
                        if (!str_cmp (argg, "show"))
                        {
                          char *l = cur_line;
                          l = one_argy (l, argg);
                          ...
                          if (!str_cmp (argg, "show_to_room"))
                          {
                            ...
                            if (scr->mob)
                            {
                              ...
                              {		/* Warning.. should ONLY be used for pset, etc.. not recho! */
                                ...
                                interpret (sch, cur_line); extract_char (sch, TRUE);
                              }

  /* End interpretting commands */

  executed_lines++; if (executed_lines > 300)
  {
...
{
  char *s; s = argy; while (*s != '\0')
  {
    if (*s != ' ')
    {
      *s = UPPER (*s); while (*s != ' ' && *s != '\0') s++;
    }
    else
    {
      s++;
...
char *string_replace (char *orig, char *old, char *new)
{
  ...
  int i;
  xbuf[0] = '\0';
  strcpy (xbuf, orig); if (strstr (orig, old) != NULL)
  {
    i = strlen (orig) - strlen (strstr (orig, old));
    xbuf[i] = '\0';
    strcat (xbuf, new);
    strcat (xbuf, &orig[i + strlen (old)]); free_string (orig);
  }
  return str_dup (xbuf);
}
...
void string_edit (CHAR_DATA * ch, char **pString)
{
  ...
  {
    free_string (*pString); *pString = str_dup ("");
  }
  ch->desc->pString = pString;
  return;
}

void phedit (CHAR_DATA * ch, char **pString)
{
  HELP_DATA * temp;

  send_to_char ("------------------------ HELP Editor -------------------------\n\r", ch);
  send_to_char (" Type .c to clear the whole text, .f to format the text,\n\r", ch);
  send_to_char (" .r 'orig' 'new' to replace, and .s to vview the string so far.\n\r", ch);
  send_to_char ("--------------------------------------------------------------\n\r", ch);
  ...
  send_to_char ("Editing help:\n\r", ch);
  send_edit_char (*pString, ch);
  send_to_char
    ("-====================================-\n\r", ch);
  temp = (HELP_DATA *) &ch->desc->pEdit;
  temp->text = *pString;
  return;
}
...
void string_add (CHAR_DATA * ch, char *argy)
{
  ...
  if (!ch->desc)
    return;
  if (!ch->pcdata->name)
    return; if (!argy || argy == "") return; if (*argy == ':')
    {
      char texts[202][255];
      char *lne;
      char *t;
      ...
      *t = '\0';
      ...
      for (t = *ch->desc->pString; *t != '\0'; t++)
      {
        if (*t == ':' || t == *ch->desc->pString)
        {
          ...
          if (curline >= 0)
            texts[curline][i] = '\0'; curline++; i = 0; continue;
        }
        if (curline < 0)
          curline = 0;
        if (*t == '\r')
          continue;
        if (*t == '\n') continue; texts[curline][i] = *t; i++;
      }
      texts[curline][i] = '\0';
      ...
      strcpy (texts[linenum - 1], argy); curline++;
      /* Now reconstruct in the char buffer */
      buf[0] = '\0'; for (i = 0; i <= curline; i++)
      {
  ...
  if (*argy == '.')
  {
    ...
    if (!str_cmp (arg1, ".c"))
    {
      if (is_number (arg2))
      {
        ...
        buf[0] = '\0'; if (linenum < 1 || linenum > 199)
        {
          send_to_char ("Invalid line number!\n\r", ch);
          return;
        }
        ...
        texts[curline][i] = '\0';
        buf[0] = '\0'; for (i = linenum; i <= curline; i++)
        {
          strcpy (texts[i - 1], texts[i]);
        }
        texts[i][0] = '\0';
        curline--; buf[0] = '\0'; for (i = 0; i <= curline; i++)
        {
          sprintf (buf + strlen (buf), ":%d ", i + 1);
          strcat (buf, texts[i]);
        }
    ...
    if (!str_cmp (arg1, ".h"))
    {
      ...
      send_to_char
        (".r 'old text' 'new text' - replace a string with a new string \n\r",
         ch);
      send_to_char (" (requires '' OR \"\" around the argys)\n\r",
          ch); send_to_char (".h - this info \n\r", ch);
          send_to_char (".s - show the string \n\r", ch);
          send_to_char
            (".f - format (word wrap/capitalize and space) string \n\r",
             ch);
  ...
  if (*argy == '~' || *argy == '@')
  {
    ...
    // save helps but only if ch has a personal help... Eraser '03
    for (pHelp = help_first; pHelp && pHelp != NULL; pHelp = pHelp->next)
    {
      if (is_name (NAME(ch), pHelp->keyword))
      {
        fnd = TRUE;
        break;
      }
  ...
  if (LEVEL (ch) < 110 && *ch->desc->pString
      && strlen (*ch->desc->pString) > 5000)
  {
  ...
  strcpy (buf, *ch->desc->pString); if (strlen (buf) > 4096)
  {
...
char *format_string (char *oldstring)
{
  ...
  int i = 0;
  bool zcap = TRUE;
  xbuf[0] = xbuf2[0] = 0; i = 0; for (t = oldstring; *t; t++)
  {
    if (*t == ':' && *(t + 1) >= '0' && *(t + 1) <= '9')
    {
      xbuf[0] = '\0'; i = 0; for (t = oldstring; *t; t++)
      {
        if (*t == '\n' || *t == '\r') continue; xbuf[i] = *t; i++;
      }
      xbuf[i] = '\n';
      xbuf[i + 1] = '\r';
      xbuf[i + 2] = '\0';
          ...
          if (*rdesc == ')')
          {
            if (xbuf[i - 1] == ' ' && xbuf[i - 2] == ' ' &&
                (xbuf[i - 3] == '.' || xbuf[i - 3] == '?'
                 || xbuf[i - 3] == '!'))
            {
              xbuf[i - 2] = *rdesc; xbuf[i - 1] = ' '; xbuf[i] = ' '; i++;
            }
            else
            {
              xbuf[i] = *rdesc; i++;
                ...
                || *rdesc == '!')
            {
                   ...
                   || xbuf[i - 3] == '!'))
              {
                ...
                {
                  xbuf[i - 1] = '\"';
                  xbuf[i] = ' '; xbuf[i + 1] = ' '; i += 2; rdesc++;
                }
              }
              else
              {
                xbuf[i] = *rdesc; if (*(rdesc + 1) != '\"')
                {
                  xbuf[i + 1] = ' '; xbuf[i + 2] = ' '; i += 3;
                }
                else
                {
                  xbuf[i + 1] = '\"';
                  xbuf[i + 2] = ' '; xbuf[i + 3] = ' '; i += 4; rdesc++;
                }
              }
              zcap = TRUE;
            }
            ...
            {
              xbuf[i] = *rdesc; if (zcap)
              {
                zcap = FALSE; xbuf[i] = UPPER (xbuf[i]);
              }
              i++;
            }
  }
  xbuf[i] = 0;
  strcpy (xbuf2, xbuf); rdesc = xbuf2; xbuf[0] = 0; for (;;)
  {
    for (i = 0; i < 77; i++)
    {
      if (!*(rdesc + i)) break;
    ...
    if (i)
    {
      *(rdesc + i) = 0;
      strcat (xbuf, rdesc);
      strcat (xbuf, "\n\r");
      rdesc += i + 1; while (*rdesc == ' ') rdesc++;
    }
    else
    {
      bug ("No spaces", 0);
      *(rdesc + 75) = 0;
      strcat (xbuf, rdesc); strcat (xbuf, "-\n\r"); rdesc += 76;
    }
  }
...
bool one_is_of_two (char *looking_for, char *line)
{
  char arg[8000];
  char *r; arg[0] = '\0'; while (line[0] != '\0')
  {
    line = one_argy (line, arg);
    r = arg;
    if (!str_cmp (looking_for, r) ||
        (strlen (looking_for) > 4
         && !str_prefix (looking_for, r))
        || (strlen (looking_for) > 4
          && !str_prefix (r, looking_for))
        || !str_infix (r, looking_for)) return TRUE;
  }
  return FALSE;
}

bool one_is_in_list_two (char *looking_for, char *line)
{
  char arg[8000];
  char *r; arg[0] = '\0'; while (line[0] != '\0')
  {
    line = one_argy (line, arg);
    r = arg; if (!str_cmp (looking_for, r)) return TRUE;
  }
  return FALSE;
}
