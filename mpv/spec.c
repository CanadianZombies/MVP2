...
spec_barbarian (CHAR_DATA * ch)
{
  ...
  if (ch->fgt && ch->fgt->ears > 9)
    return FALSE;
  if (number_range (1, 3) == 1 && LEVEL (ch) > 25)
    {
      do_slam (ch, "");
      return TRUE;
    }
  else if (number_range (1, 3) == 1 && LEVEL (ch) > 10)
    {
      do_headbutt (ch, "");
      return TRUE;
    }
...
spec_eatcorpse (CHAR_DATA * ch)
{
  ...
  for (corpse = ch->in_room->more->contents; corpse != NULL; corpse = c_next)
	...
	continue;
      act ("$n begins munching away at the corpse!!", ch, NULL, NULL,
	   TO_ROOM);
...
spec_guard (CHAR_DATA * ch)
{
  ...
  if (ch->in_room == NULL)
    return FALSE;
  for (victim = ch->in_room->more->people; victim != NULL;
       victim = victim->next_in_room)
    {
      if (IS_PLAYER (victim) && victim->pcdata->bounty > 20)
	break;
    }
  if (victim != NULL)
    {
      if (IS_MOB (victim))
	return FALSE;
      /*change next line if guards should kill outlaws in no-jail zones */
      if (get_room_index (victim->in_room->area->jail) == NULL)
	return FALSE;
      if (number_range (1, 3) == 2)
	{
	  if (victim->pcdata->warned < 5)
	    {
	      sprintf (buf, "%s is an outlaw! Surrender or DIE!",
		       NAME (victim));
	      do_say (ch, buf);
	      victim->pcdata->warned++;
	      return TRUE;
	    }
	  if (victim->pcdata->warned == 5)
	    {
	      sprintf (buf, "This is your last warning, %s! Surrender NOW!",
		       NAME (victim));
	      do_say (ch, buf);
	      victim->pcdata->warned++;
	      return TRUE;
	  ...
	  if (victim->pcdata->warned > 5)
	    {
	      if (FIGHTING (ch) == NULL)
		{
		  sprintf (buf, "Prepare to die, outlaw!");
		  do_say (ch, buf);
		  multi_hit (ch, victim, TYPE_UNDEFINED);
		}
	      return TRUE;
...
spec_janitor (CHAR_DATA * ch)
{
  ...
  for (trash = ch->in_room->more->contents; trash != NULL; trash = trash_next)
	...
	continue;
	  ...
	  || trash->pIndexData->item_type == ITEM_TRASH || trash->cost < 10)
	{
  ...
  for (victim = ch->in_room->more->people; victim != NULL; victim = v_next)
    {
      ...
      if (IS_MOB (victim))
	continue;
      if (LEVEL (victim) >= LEVEL_IMMORTAL || number_bits (3) != 0 || !can_see (ch, victim))	/* Thx Glop */
	continue;
      if (is_safe (ch, victim))
	return FALSE;
      if (IS_SET (ch->act, ACT_ATTITUDE)
	  && (!IS_AWAKE (victim) || number_range (1, 11) == 2))
	{
			     ...
			     get_curr_dex (victim))))
	{
	  ...
	  for (obj2 = victim->carrying; obj2 != NULL;
	       obj2 = obj2->next_content)
	    {
	      if ((obj2->wear_loc != -1) || !can_drop_obj (victim, obj2))
		continue;
	      if (number_percent () < chance)
		{
		  ...
		  obj_to (obj2, ch);
		  if (dice (1, 6) <
		      (get_curr_dex (victim) - get_curr_dex (ch)))
