...
/* wait if command already here */
  ...
  for (;;)
...
#else
      nRead = read (d->descriptor, d->inbuf + iStart, 500);
#endif
      ...
      if (nRead > 0)
	{
	  ...
	  if (d->host[29] == 1 && d->connected != CON_FSEDITOR &&
	      ...
	      for (t = &d->inbuf[iStart - nRead]; *t != '\0'; t++)
		{
		  ...
		  while (*t == '\b' || *t == 16 || *t == 127)
		    {
		      if (d->character
			  && d->character->pcdata->convo[199] > 0)
			{
			  d->character->pcdata->convo[199]--;
	  ...
	  if (d->character
	      && (!IS_SET (d->character->pcdata->act2, PLR_TELNET_GA)))
	    {
	      ...
	      if (d->inbuf[iStart - 1] == 18)
		{
	      ...
	      if ((d->inbuf[iStart - 1] == '\b' || d->inbuf[iStart - 1] == 127
		   || d->inbuf[iStart - 1] == 16)
		  && d->connected == CON_FSEDITOR)
		{
		  if (nRead > 1
		      && (d->inbuf[iStart - 2] == '\b'
			  || d->inbuf[iStart - 2] == 127))
		    {
		      d->inbuf[0] = 18;
		      d->inbuf[1] = '\0';
		      character_add (d->character, d->inbuf);
      ...
      else if (nRead == 0 || nRead > 1000)
	{
	  ...
	  sprintf (buf, "Notify> %s has lost %s link.",
		   (d
		    && d->character) ? d->character->pcdata->
		   name : "New_char", (d
				       && d->character) ? HIS_HER (d->
								   character)
		   : "its");
