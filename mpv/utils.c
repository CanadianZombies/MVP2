...
void do_cleanup (void)
{
  ...
  for (pb = playerbase_zero; pb != NULL; pb = pb->next)
	...
	continue;
      if (pb->player_level == -1)
	continue;
      sprintf (buf, "mv %s%s.cor cleandir", PLAYER_DIR,
	       capitalize (pb->player_name));
      system (buf);
      sprintf (buf, "mv %s%s.tro cleandir", PLAYER_DIR,
	       capitalize (pb->player_name));
      system (buf);
      sprintf (buf, "mv %s%s cleandir", PLAYER_DIR,
	       capitalize (pb->player_name));
      system (buf);
    }
...
int main (int argc, char **argv)
{
  ...
  int i;
  fprintf (stderr,
	   "Utils for Owen Emlen's Aturion Dynasty code, written by Owen Emlen.\n");
  ...
  if (!found)
    {
      ...
      fprintf (stderr,
	       "-cleanup     removes all people not on record in the pbase.\n");
      fprintf (stderr,
	       "-deleteold   removes all people who haven't played in a while.\n");
      return 1;
    }
