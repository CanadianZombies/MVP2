...
do_drag (CHAR_DATA * ch, char *argy)
{
  ...
  int dir;
  ROOM_DATA *oldroom;
  DEFINE_COMMAND ("drag", do_drag, POSITION_STANDING, 0, LOG_NORMAL, "This command allows you to drag a corpse.") arg1[0] = '\0';
  arg2[0] = '\0';
  if (strlen (argy) >= 50)
    return;
  ...
  if ((obj = get_obj_here (ch, arg1, SEARCH_ROOM_FIRST)) == NULL
      || obj->carried_by != NULL || obj->in_room == NULL)
    {
  ...
  sprintf (buf, "%s drags %s %s.\n\r", NAME (ch), (obj->short_descr ? obj->short_descr : obj->pIndexData-> short_descr), dir_name[dir]);
  if (oldroom->more)
  ...
  sprintf (buf, "You drag %s into the room.\n\r", (obj->short_descr ? obj->short_descr : obj->pIndexData->short_descr));
  send_to_char (buf, ch);
  sprintf (buf, "%s drags %s in from %s.\n\r", NAME (ch), (obj->short_descr ? obj->short_descr : obj->pIndexData->short_descr), dir_rev[rev_dir[dir]]);
...
{
  DEFINE_COMMAND ("meditate", do_meditate, POSITION_RESTING, 0, LOG_NORMAL,
		  "Allows you to meditate, and focus on magical energy regeneration.")
    if (RIDING (ch) != NULL)
    {
      send_to_char
	("If you wanted to sit and meditate, you would not be riding!\n\r",
	 ch);
      return;
    }
  ...
  if (FIGHTING (ch))
    {
      act ("You can not meditate while fighting!! Are you nuts?", ch, NULL, NULL, TO_CHAR);
      return;
    }
  if (ch->position != POSITION_STANDING && ch->position != POSITION_RESTING && ch->position != POSITION_MEDITATING && ch->position != POSITION_STOPCAST)
    {
...
void
get_obj (CHAR_DATA * ch, SINGLE_OBJECT * obj, SINGLE_OBJECT * container)
{
  CHAR_DATA *fch;
  CHAR_DATA *fch_next;
#ifndef NEW_WORLD
  if (obj->pIndexData->item_type == ITEM_BOAT && !is_boat_empty (obj))
    {
      send_to_char
	("The boat has people inside! It's just too heavy to carry...\n\r",
	 ch);
      return;
    }
#endif
  ...
  if (container != NULL)
    {
      act ("You get $p\x1B[37;0m from $P\x1B[0m.", ch, obj, container,
	   TO_CHAR);
      act ("$n gets $p\x1B[37;0m from $P\x1B[0m.", ch, obj, container,
	   TO_ROOM);
    }
  else
    {
...
{
  DEFINE_COMMAND ("take", do_get, POSITION_RESTING, 0, LOG_NORMAL,
		  "This command allows you to get an object from an object, or from the ground, or to 'take control' of a boat.")
    return;
}*/
...
do_get (CHAR_DATA * ch, char *argy)
{
  ...
  bool found = FALSE;
  bool gonnasave;
  DEFINE_COMMAND ("get", do_get, POSITION_RESTING, 0, LOG_NORMAL,
		  "This command allows you to get an object from an object, or from the ground.")
    check_room_more (ch->in_room);

  gonnasave = FALSE;
#ifndef NEW_WORLD
  if (ch->ced && ch->ced->in_boat != NULL && ch->ced->in_boat->boat
      && !str_cmp (argy, "control"))
    {
      char buffr[500];
      if (!ch->ced->in_boat->boat || ch->ced->in_boat->boat->navigator == ch)
	{
	  send_to_char ("You are already the navigator of the vessel!\n\r",
			ch);
	  return;
	}
      if (!IS_EVIL (ch) && ch->ced->in_boat->boat->navigator != NULL
	  && IS_EVIL (ch->ced->in_boat->boat->navigator))
	{
	  send_to_char
	    ("You'll have to kill the current navigator first...\n\r", ch);
	  return;
	}
      if (IS_EVIL (ch) && ch->ced->in_boat->boat->navigator != NULL
	  && !IS_EVIL (ch->ced->in_boat->boat->navigator))
	{
      ...
      if (ch->ced->in_boat->boat->navigator == NULL)
	{
	  send_to_char ("You take the helm of the vessel...\n\r", ch);
	  sprintf (buffr, "[%s takes control of the boat's navigation]\n\r",
		   NAME (ch));
	  send_to_boat (ch, ch->ced->in_boat, buffr);
	}
      else
	{
	  send_to_char ("You take the helm of the vessel...\n\r", ch);
	  sprintf (buffr,
		   "[%s takes a breather as %s steps up to the vessel's helm]\n\r",
		   NAME (ch->ced->in_boat->boat->navigator), NAME (ch));
	  send_to_boat (ch, ch->ced->in_boat, buffr);
	}
      ch->ced->in_boat->boat->navigator = ch;
      return;
    }
#endif
  if (!str_prefix ("cont", argy)
      ...
      && ch->in_room->more->linked_to)
    {
      ...
      sprintf (buffr, "%s takes control of the vehicle's navigation.",
	       NAME (ch));
      act (buffr, ch, NULL, ch, TO_ROOM);
      rs = ch->in_room;
      char_from_room (ch);
      char_to_room (ch, rs);
      return;
  ...
  if (arg2[0] == '\0')
      ...
      if (str_cmp (arg1, "all") && str_prefix ("all.", arg1))
	{
	  if (!str_cmp (arg1, "coins") || !str_cmp (arg1, "coin"))
	    {
	      if (ch->in_room->more->copper < 1
		  && ch->in_room->more->gold < 1)
		{
		...
		sprintf (bah,
			 "You get %d \x1B[33;0mcopper\x1B[37m coin%s.\n\r",
			 ch->in_room->more->copper,
			 (ch->in_room->more->copper > 1 ? "s" : ""));
	      else if (ch->in_room->more->gold > 0
		       && ch->in_room->more->copper == 0)
		sprintf (bah,
			 "You get %d \x1B[33;1mgold\x1B[37;0m coin%s.\n\r",
			 ch->in_room->more->gold,
			 (ch->in_room->more->gold > 1 ? "s" : ""));
	      else if (ch->in_room->more->gold > 0
		       && ch->in_room->more->copper > 0)
		sprintf (bah,
			 "You get %d \x1B[33;0mcopper\x1B[37m coin%s and %d \x1B[33;1mgold\x1B[37;0m coin%s.\n\r",
			 ch->in_room->more->copper,
			 (ch->in_room->more->copper > 1 ? "s" : ""),
			 ch->in_room->more->gold,
			 (ch->in_room->more->gold > 1 ? "s" : ""));
	  ...
	  for (obj = ch->in_room->more->contents; obj != NULL; obj = obj_next)
	    {
	      ...
	      if ((arg1[3] == '\0'
		   || is_name (&arg1[4], obj->pIndexData->name))
		  && can_see_obj (ch, obj)
      ...
      if (!str_cmp (arg2, "all") || !str_prefix ("all.", arg2))
	{
      ...
      if ((container =
	   (!str_prefix ("cor", arg2) ?
	    get_obj_here (ch, arg2, SEARCH_ROOM_FIRST) : get_obj_here (ch,
								       arg2,
								       SEARCH_INV_FIRST)))
	  == NULL)
	{
	...
	case ITEM_CORPSE_NPC:
	  {
	    ...
	    if ((cnt->money > 0 || container->contains)
		&& strlen (NAME (ch)) < 30)
	      {
		...
		else if (cnt->looted_by_two[0] == '\0'
			 && str_cmp (NAME (ch), cnt->looted_by))
		  {
	...
	case ITEM_CORPSE_PC:
	  {
	    ...
	    if (!pow.get_from_corpse_while_in_combat && FIGHTING (ch))
	      {
		send_to_char
		  ("You don't have time to loot the corpse while you are fighting!\n\r",
		   ch);
		return;
	      }
	    cnt = (I_CONTAINER *) container->more;
	    ...
	    if (IS_PLAYER (ch) && LEVEL (ch) <= pow.loot_level
		&& str_cmp (name, NAME (ch)))
	      {
		send_to_char
		  ("You cannot loot another person's corpse at that low level.\n\r",
		   ch);
		return;
	      }
	    if ((container->contains || cnt->money > 0)
		&& str_cmp (NAME (ch), capitalize (name)))
	      {
		...
		fprintf (stderr,
			 "%s just took something from %s's corpse.\n",
	...
	{
	  act ("The $d is closed.", ch, NULL, container->pIndexData->name,
	       TO_CHAR);
	  return;
	}
      if (str_cmp (arg1, "all") && str_prefix ("all.", arg1))
	{
	  ...
	  if (!str_cmp (arg1, "coins"))
	      ...
	      if (cnt->money < 1)
		{
	      ...
	      sprintf (bah, "You get %d copper from %s\x1B[37;0m.\n\r",
		       cnt->money, OOSTR (container, short_descr));
	      ch->copper += cnt->money;
	      cnt->money = 0;
	      send_to_char (bah, ch);
	      return;
	  ...
	  if (obj == NULL)
	    {
	      act ("I see nothing like that in the $T\x1B[0m.", ch, NULL,
		   arg2, TO_CHAR);
	      return;
	    }
      ...
      else
	{
	  ...
	  if (cnt->money > 0)
	    {
	      sprintf (bah, "You get %s from %s.\x1B[37;0m\n\r",
		       name_amount (cnt->money), OOSTR (container,
							short_descr));
	      ch->copper += cnt->money;
	      cnt->money = 0;
	      send_to_char (bah, ch);
	  ...
	  if (!found)
		...
		act ("I see nothing like that in $T\x1B[0m.", ch, NULL, arg2,
		     TO_CHAR);
	    }
	}
    }
/*if (IS_PLAYER(ch) && gonnasave) save_char_obj(ch); */
  if (IS_PLAYER (ch))
...
do_put (CHAR_DATA * ch, char *argy)
{
  ...
  SINGLE_OBJECT *obj_next;
  I_CONTAINER *cnt;
  DEFINE_COMMAND ("put", do_put, POSITION_RESTING, 0, LOG_NORMAL,
		  "This command allows you to put an object in something.")
    argy = one_argy (argy, arg1);
  ...
  if (!str_cmp (arg1, "all.coin") || !str_cmp (arg1, "coin")
      || !str_cmp (arg1, "all.coins") || !str_cmp (arg1, "coins"))
    {
      char buffu[500];
      ...
      if (container->pIndexData->item_type != ITEM_CONTAINER)
	{
      ...
      cnt->money += ch->copper;
      sprintf (buffu,
	       "You put your copper in %s.  (Gold pieces can't go inside containers).\n\r",
	       OOSTR (container, short_descr));
  ...
  if (str_cmp (arg1, "all") && str_prefix ("all.", arg1))
      ...
      if ((obj = get_obj_inv (ch, arg1)) == NULL)
	{
      ...
      if (obj->pIndexData->vnum < MAX_LIMITED
	  && obj->pIndexData->how_many > 0)
	{
	  send_to_char
	    ("This item doesn't seem to fit inside the container.\n\r", ch);
	  return;
	}
      ...
      for (obj = ch->carrying; obj != NULL; obj = obj_next)
	{
	  ...
	  if ((arg1[3] == '\0' || is_name (&arg1[4], obj->pIndexData->name))
	      ...
	      <= cnt->max_weight)
	    {
	      obj_from (obj);
	      obj_to (obj, container);
	      act ("$n puts $p\x1B[0m in $P\x1B[0m.", ch, obj, container,
		   TO_ROOM);
	      act ("You put $p\x1B[0m in $P\x1B[0m.", ch, obj, container,
		   TO_CHAR);
	    }
	}
    }
...
do_drop (CHAR_DATA * ch, char *argy)
{
  ...
  bool found = FALSE;
  char arg2[SML_LENGTH];
  DEFINE_COMMAND ("drop", do_drop, POSITION_RESTING, 0, LOG_NORMAL,
		  "This command allows you to drop an object.") argy =
    one_argy (argy, arg);
  if (!is_number (arg))
    {
      while (argy != "" && argy[0] != '\0')
  ...
  CAN_DROP_ITEMS_IN_BGROUND (no)
    CAN_DROP_ITEMS_IN_ARENA (no) if (arg[0] == '\0')
    {
      send_to_char ("Drop what?\n\r", ch);
      return;
    }
  if (!str_cmp (arg, "coin") || !str_cmp (arg, "coins")
      || !str_cmp (arg, "all.coin") || !str_cmp (arg, "all.coins"))
    {
      char bufff[500];
      if (tally_coins (ch) < 0)
	{
  ...
  if ((!str_cmp (arg2, "gold")) && (is_number (arg) && atoi (arg) > 0))
    {
      amount = atoi (arg);
      if (amount < 1)
	return;
	...
	{
	  send_to_char
	    ("You don't have that many \x1B[33;1mgold\x1B[37;0m coins.\n\r",
	     ch);
	  return;
	}
      ch->gold -= amount;
	...
	ch->in_room->more->gold += amount;
      sprintf (buf, "You drop %d \x1B[33;1mgold\x1B[37;0m coins.\n\r",
	       amount);
      send_to_char (buf, ch);
      act ("$n drops some \x1B[33;1mgold\x1B[37;0m coins.", ch, NULL, NULL,
	   TO_ROOM);
      return;
    }

  if ((!str_cmp (arg2, "copper")) && (is_number (arg) && atoi (arg) > 0))
    {
      amount = atoi (arg);
      if (amount < 1)
	return;
	...
	{
	  send_to_char
	    ("You don't have that many \x1B[33mcopper\x1B[37;0m coins.\n\r",
	     ch);
	  return;
	}
      ch->copper -= amount;
	...
	ch->in_room->more->copper += amount;
      sprintf (buf, "You drop %d \x1B[33mcopper\x1B[37;0m coins.\n\r",
	       amount);
      send_to_char (buf, ch);
      act ("$n drops some \x1B[33mcopper\x1B[37;0m coins.", ch, NULL, NULL,
	   TO_ROOM);
      return;
    }
  if ((str_cmp (arg, "all") && str_prefix ("all.", arg))
	  ...
	  (ch->fgt && ch->fgt->combat_delay_count == 9))
	{
	  ...
	  if (ch->in_room->sector_type == SECT_WATER_SWIM
	      || ch->in_room->sector_type == SECT_WATER_NOSWIM)
	    {
	      act ("The waters cause $p to sink beneath them; lost forever.",
		   ch, obj, NULL, TO_CHAR);
	      free_it (obj);
	      return;
	    }
	...
	{
	  BLIST *bd;
	  bd = mem_alloc (sizeof (*bd));
	  bd->next = base_blist;
	  base_blist = bd;
	  bd->vnum = ch->in_room->vnum;
	  save_boats ();
	}
	  ...
	  && (ch->in_room->sector_type == SECT_WATER_SWIM
	      || ch->in_room->sector_type == SECT_WATER_NOSWIM))
	{
	  act ("The waters cause $p to sink beneath them; lost forever.", ch,
	       obj, NULL, TO_CHAR);
	  free_it (obj);
	}
      else
		...
		if (obj->pIndexData->vnum == tr->attached_to_obj)
		  {
		      ...
		      }
		    {
		      ...
		      if (tr->keywords[0] != '\0'
			  && !one_is_in_list_two (tmp, tr->keywords))
			continue;
		    }
		    /* ----------------- */
		    /* Start the script! */
...
/* End trigger check! */
      ...
      for (obj = ch->carrying; obj != NULL; obj = obj_next)
	{
	  ...
	  if ((arg[3] == '\0' || is_name (&arg[4], obj->pIndexData->name))
		...
		{
		  act
		    ("The waters cause $p to sink beneath them; lost forever.",
		     ch, obj, NULL, TO_CHAR);
		  free_it (obj);
		}
	      else
...
/* Dropped item, check for triggers on object! */
		  {
		    SINGLE_TRIGGER *tr;
		    SCRIPT_INFO *s;
		    for (tr = trigger_list[TDROPPED]; tr != NULL;
			 tr = tr->next)
		      {
			if (obj->pIndexData->vnum == tr->attached_to_obj)
			  {
...
/* End trigger check! */
      ...
      if (!found)
	{
	  if (arg[3] == '\0')
	    ...
	    act ("You are not carrying $T.", ch, NULL, &arg[4], TO_CHAR);
	}
    }
  return;
}
...
do_give (CHAR_DATA * ch, char *argy)
{
  ...
  int amount;
  char buf[STD_LENGTH];
  DEFINE_COMMAND ("give", do_give, POSITION_RESTING, 0, LOG_NORMAL,
		  "This command allows you to give something to someone.")
    BUILDERS_CAN_GIVE (no) if (FIGHTING (ch) && !pow.give_while_fighting)
    {
  ...
  CAN_GIVE_ITEMS_IN_ARENA (no)
    CAN_GIVE_ITEMS_IN_BGROUND (no) argy = one_argy (argy, arg1);
  argy = one_argy (argy, arg2);
  ...
  if ((!str_cmp (arg2, "gold")) && (is_number (arg1) && atoi (arg1) > 0))
    {
      amount = atoi (arg1);
      if (amount < 1)
	return;
	...
	return;
      ...
      sprintf (buf, "You give %d \x1B[33;1mgold\x1B[37;0m coins to %s.\n\r",
	       amount, NAME (victim));
      send_to_char (buf, ch);
      act ("$n gives some \x1B[33;1mgold\x1B[37;0m to $N.", ch, NULL, victim,
	   TO_NOTVICT);
...
/*     if ( IS_MOB(victim) && victim->npcdata->hire == ch )
  ...
  if ((!str_cmp (arg2, "copper")) && (is_number (arg1) && atoi (arg1) > 0))
    {
      amount = atoi (arg1);
      if (amount < 1)
	return;
	...
	return;
      ...
      sprintf (buf, "You give %d \x1B[33mcopper\x1B[37;0m coins to %s.\n\r",
	       amount, NAME (victim));
      send_to_char (buf, ch);
      act ("$n gives some \x1B[33mcopper\x1B[37;0m to $N.", ch, NULL, victim,
	   TO_NOTVICT);
...
/* End trigger check! */
  ...
  if (IS_MOB (victim))
    check_given_script (victim, ch, obj->pIndexData->vnum);
  if (IS_MOB (victim))
    {
      char tmp[50];
      sprintf (tmp, "%d", obj->pIndexData->vnum);
      hardcode_give_scripts (ch, victim, tmp);
    }
...
do_fill (CHAR_DATA * ch, char *argy)
{
  ...
  I_DRINK *dr;
  bool found;
  DEFINE_COMMAND ("fill", do_fill, POSITION_STANDING, 0, LOG_NORMAL,
		  "This command allows you to fill a container or lamp.") argy
    = one_argy (argy, arg);
  if (arg[0] == '\0')
    {
...
/*held */
  ...
  if (obj->pIndexData->item_type == ITEM_DRINK_CON)
	...
	{
	  send_to_char
	    ("You don't have anything in your hand to fill it with.\n\r", ch);
	  return;
	}
      drk = (I_DRINK *) obj2->more;
      if (!IS_SET (lg->light_lit, LIGHT_FILLABLE)
	  && !IS_SET (lg->light_fillable, LIGHT_FILLABLE))
	{
	  send_to_char ("It can't be refueled.\n\r", ch);
	  return;
	}
      if (drk->liquid_type != -1)
	{
...
do_empty (CHAR_DATA * ch, char *argy)
{
  char arg[SML_LENGTH];
  SINGLE_OBJECT *obj;
  DEFINE_COMMAND ("empty", do_empty, POSITION_RESTING, 0, LOG_NORMAL,
		  "This command allows you to empty a container of liquid.")
    argy = one_argy (argy, arg);
  if (arg[0] == '\0')
    {
...
do_drink (CHAR_DATA * ch, char *argy)
{
  ...
  bool thirsty;
  int liquid;
  DEFINE_COMMAND ("drink", do_drink, POSITION_RESTING, 0, LOG_NORMAL,
		  "This command allows you to drink from something.")
    while (argy != "" && argy[0] != '\0')
    argy = one_argy (argy, arg);
  ...
  if (IS_PLAYER (ch) && ch->pcdata->condition[COND_DRUNK] > 10)
    {
      send_to_char
	("You are too drunk, dazed, and confused to drink any more.\n\r", ch);
      return;
    }
  ...
  switch (obj->pIndexData->item_type)
	...
	if (liquid == -1)
	...
	else
	  {
		...
		send_to_char
		  ("Your intestines feel like they are on fire!\n\r", ch);
		af.type = gsn_poison;
...
#ifdef OLD_NOMOVE
  if (IS_PLAYER (ch) && IS_SET (ch->act2, PLR_BLANKPROMPT) &&
      ch->pcdata->condition[COND_THIRST] > 6 && thirsty)
    {
      if (ch->pcdata->condition[COND_FULL] > 6)
	{
	  upd_hunger (ch);
	  upd_thirst (ch);
	}
...
do_eat (CHAR_DATA * ch, char *argy)
{
  char arg[SML_LENGTH];
  SINGLE_OBJECT *obj;
  DEFINE_COMMAND ("eat", do_eat, POSITION_RESTING, 0, LOG_NORMAL,
		  "This command allows you to eat something.") argy =
    one_argy (argy, arg);
  ...
  if (!IS_IMMORTAL (ch))
	  ...
	  && obj->pIndexData->item_type != ITEM_PILL)
	{
      ...
      if (IS_PLAYER (ch) && obj->pIndexData->item_type != ITEM_PILL
	  && !NO_HUNGER (ch) && ch->pcdata->condition[COND_FULL] > 40)
	{
	...
	if (fd->not_poison != 0)
	  {
	    ...
	    AFFECT_DATA af;
	    bzero (&af, sizeof (af));
	    act ("$n's face pales, and $e clutches $s stomach.", ch, 0, 0,
		 TO_ROOM);
  ...
  if (IS_PLAYER (ch) && obj->pIndexData->item_type == ITEM_FOOD)
    {
#ifdef OLD_NOMOVE
      upd_hungert (ch);
#endif
...
draw_obj (CHAR_DATA * ch, SINGLE_OBJECT * obj)
{
  if (obj->wear_loc != WEAR_BELT_1
      ...
      && obj->wear_loc != WEAR_BELT_4 && obj->wear_loc != WEAR_BELT_5)
    return FALSE;
  ...
  if (IS_MOB (ch) && (pShop = ch->pIndexData->pShop) != NULL)
      ...
      if (CAN_WEAR (obj, ITEM_WEAR_FINGER))
	{
	  if (loc == 0 || loc == ITEM_WEAR_FINGER)
		...
		return;
	      if (get_eq_char (ch, WEAR_FINGER_L) == NULL)
		{
		  act ("$n slips $p\x1B[0m onto $s left finger.", ch, obj,
		       NULL, TO_ROOM);
		  act ("You slip $p\x1B[0m onto your left finger.", ch, obj,
		       NULL, TO_CHAR);
		...
		{
		  act ("$n slips $p onto $s right finger.", ch, obj, NULL,
		       TO_ROOM);
		  act ("You slip $p onto your right finger.", ch, obj, NULL,
		       TO_CHAR);
      ...
      if (CAN_WEAR (obj, ITEM_WEAR_NECK))
	{
	  if (loc == 0 || loc == ITEM_WEAR_NECK)
		...
		return;
	      if (get_eq_char (ch, WEAR_NECK_1) == NULL)
		{
		  act ("$n clasps $p\x1B[0m around $s neck.", ch, obj, NULL,
		       TO_ROOM);
		  act ("You clasp $p\x1B[0m around your neck.", ch, obj, NULL,
		       TO_CHAR);
      ...
      if (CAN_WEAR (obj, ITEM_FLOATING))
	{
	  if (loc == 0 || loc == ITEM_FLOATING)
	    {
	      if (get_eq_char (ch, WEAR_FLOAT_1) != NULL
		  && get_eq_char (ch, WEAR_FLOAT_2) != NULL
		  && !remove_obj (ch, WEAR_FLOAT_1, fReplace)
		  && !remove_obj (ch, WEAR_FLOAT_2, fReplace))
		return;
	      if (!get_eq_char (ch, WEAR_FLOAT_1))
		{
		  act ("$p begins floating nearby $n's head.", ch, obj, NULL,
		       TO_ROOM);
		  act ("$p begins floating nearby your head.", ch, obj, NULL,
		       TO_CHAR);
		  equip_char (ch, obj, WEAR_FLOAT_1);
		  return;
		}
	      if (!get_eq_char (ch, WEAR_FLOAT_2))
		{
		  act ("$p begins floating nearby $n's head.", ch, obj, NULL,
		       TO_ROOM);
		  act ("$p begins floating nearby your head.", ch, obj, NULL,
	      ...
	      bug ("Wear_obj: no free float space.", 0);
	      send_to_char ("You already have 2 floaters.\n\r", ch);
	      return;
	    }
	}
          ...
          if (loc == 0 || loc == ITEM_WEAR_KNEE)
            {
              if (get_eq_char (ch, WEAR_KNEE_1) != NULL
                ...
                return;
              if (!get_eq_char (ch, WEAR_KNEE_1))
                {
                  act ("$n straps $p on thier left knee.", ch, obj, NULL,
                       TO_ROOM);
                  act ("You strap $p to your left knee.", ch, obj, NULL,
                       TO_CHAR);
                  equip_char (ch, obj, WEAR_KNEE_1);
                  return;
                }
              if (!get_eq_char (ch, WEAR_KNEE_2))
                {
                  act ("$n straps $p on thier right knee.", ch, obj, NULL,
                       TO_ROOM);
                  act ("You strap $p to your right knee.", ch, obj, NULL,
                       TO_CHAR);
...
if (!get_eq_char (ch, WEAR_KNEE_2))
                {
                  act ("$n straps on some $p.", ch, obj, NULL,
                       TO_ROOM);
                  act ("You strap on some $p.", ch, obj, NULL,
                       TO_CHAR);
                  equip_char (ch, obj, WEAR_KNEE_2);
                  return;
                }
              bug ("Wear_obj: no free knee space.", 0);
              send_to_char ("You already have 2 knee eq on.\n\r", ch);
              return;
            }
        }
    ...
    if (CAN_WEAR (obj, ITEM_FLOATING))
    {
      if (loc == 0 || loc == ITEM_FLOATING)
      {
        if (!remove_obj (ch, WEAR_FLOATING, fReplace))
          return;
        act ("$p begins floating nearby $n's head.", ch, obj, NULL, TO_ROOM);
        act ("$p begins floating nearby your head.", ch, obj, NULL, TO_CHAR);
...
*/
      if (CAN_WEAR (obj, ITEM_WEAR_BODY))
	{
	  if (loc == 0 || loc == ITEM_WEAR_BODY)
		...
		return;
	      ...
	      act ("You wear $p\x1B[0m on your body.", ch, obj, NULL,
		   TO_CHAR);
      ...
      if (CAN_WEAR (obj, ITEM_WEAR_FACE))
	{
	  if (loc == 0 || loc == ITEM_WEAR_FACE)
		...
		return;
	      act ("$n straps $p\x1B[0m onto $s face.", ch, obj, NULL,
		   TO_ROOM);
      ...
      if (CAN_WEAR (obj, ITEM_WEAR_LEGS))
	{
	  if (loc == 0 || loc == ITEM_WEAR_LEGS)
		...
		return;
	      ...
	      act ("You wear $p\x1B[0m on your legs.", ch, obj, NULL,
		   TO_CHAR);
      ...
      if (CAN_WEAR (obj, ITEM_WEAR_FEET))
	{
	  if (loc == 0 || loc == ITEM_WEAR_FEET)
		...
		return;
	      act ("$n slips $s feet into $p\x1B[0m.", ch, obj, NULL,
		   TO_ROOM);
	      act ("You slip your feet into $p\x1B[0m.", ch, obj, NULL,
		   TO_CHAR);
      ...
      if (CAN_WEAR (obj, ITEM_WEAR_HANDS))
	{
	  if (loc == 0 || loc == ITEM_WEAR_HANDS)
		...
		return;
	      ...
	      act ("You wear $p\x1B[0m on your hands.", ch, obj, NULL,
		   TO_CHAR);
          ...
          if (loc == 0 || loc == ITEM_WEAR_TATTOO)
            {
              if (!remove_obj (ch, WEAR_TATTOO, fReplace))
                return;
              act ("$n applies $p\x1B[0m on $s upper arm.", ch, obj, NULL, TO_ROOM);
              act ("You apply $p\x1B[0m on your upper arm.", ch, obj, NULL, TO_CHAR);
      ...
      if (CAN_WEAR (obj, ITEM_WEAR_ARMS))
	{
	  if (loc == 0 || loc == ITEM_WEAR_ARMS)
		...
		return;
	      ...
	      act ("You wear $p\x1B[0m on your arms.", ch, obj, NULL,
		   TO_CHAR);
	      equip_char (ch, obj, WEAR_ARMS);
	      return;
	    }
	}
      if (CAN_WEAR (obj, ITEM_WEAR_EYEPATCH))
        {
          if (loc == 0 || loc == ITEM_WEAR_EYEPATCH)
            {
              if (!remove_obj (ch, WEAR_EYEPATCH, fReplace))
                return;
              act ("$n wears $p\x1B[0m on $s left eye.", ch, obj, NULL, TO_ROOM);
              act ("You wear $p\x1B[0m on your left eye.", ch, obj, NULL,
                   TO_CHAR);
              equip_char (ch, obj, WEAR_EYEPATCH);
              return;
            }
        }
      if (CAN_WEAR (obj, ITEM_WEAR_ABOUT))
	{
	  if (loc == 0 || loc == ITEM_WEAR_ABOUT)
		...
		return;
	      act ("$n wears $p\x1B[0m about $s body.", ch, obj, NULL,
		   TO_ROOM);
	      act ("You wear $p\x1B[0m about your body.", ch, obj, NULL,
		   TO_CHAR);
      ...
      if (CAN_WEAR (obj, ITEM_WEAR_WAIST))
	{
	  if (loc == 0 || loc == ITEM_WEAR_WAIST)
		...
		return;
	      act ("$n fastens $p\x1B[0m around $s waist.", ch, obj, NULL,
		   TO_ROOM);
	      act ("You fasten $p\x1B[0m around your waist.", ch, obj, NULL,
		   TO_CHAR);
      ...
      if (CAN_WEAR (obj, ITEM_WEAR_EAR1))
	{
	  if (loc == 0 || loc == ITEM_WEAR_EAR1)
		...
		return;
	      if (get_eq_char (ch, WEAR_EAR_1) == NULL)
		{
		  act ("$n wears $p\x1B[0m in $s right ear.", ch, obj, NULL,
		       TO_ROOM);
		  act ("You wear $p\x1B[0m in your right ear.", ch, obj, NULL,
		       TO_CHAR);
		...
		{
		  act ("$n wears $p\x1B[0m in $s left ear.", ch, obj, NULL,
		       TO_ROOM);
		  act ("You wear $p\x1B[0m in your left ear.", ch, obj, NULL,
		       TO_CHAR);
	  ...
	  if (loc == 0 || loc == ITEM_WEAR_ANKLE)
	    {
		  ...
		  && (get_eq_char (ch, WEAR_ANKLE_2) != NULL))
		return;
	      if (get_eq_char (ch, WEAR_ANKLE_1) == NULL)
		{
		  act ("$n wears $p\x1B[0m around $s right ankle.", ch, obj,
		       NULL, TO_ROOM);
		  act ("You wear $p\x1B[0m around your right ankle.", ch, obj,
		       NULL, TO_CHAR);
		...
		{
		  act ("$n wears $p\x1B[0m around $s left ankle.", ch, obj,
		       NULL, TO_ROOM);
		  act ("You wear $p\x1B[0m around your left ankle.", ch, obj,
		       NULL, TO_CHAR);
          ...
          if (loc == 0 || loc == ITEM_WEAR_ELBOW)
            {
              ...
              if (get_eq_char (ch, WEAR_ELBOW_1) == NULL)
                {
                  act ("$n wears $p\x1B[0m around $s right elbow.", ch, obj,
                       NULL, TO_ROOM);
                  act ("You wear $p\x1B[0m around your right elbow.", ch, obj,
                       NULL, TO_CHAR);
              ...
              if (get_eq_char (ch, WEAR_ELBOW_2) == NULL)
                {
                  act ("$n wears $p\x1B[0m around $s left elbow.", ch, obj,
                       NULL, TO_ROOM);
                  act ("You wear $p\x1B[0m around your left elbow.", ch, obj,
                       NULL, TO_CHAR);
      ...
      if (CAN_WEAR (obj, ITEM_WEAR_SHOULDER))
	{
	  if (loc == 0 || loc == ITEM_WEAR_SHOULDER)
	      ...
	      if (get_eq_char (ch, WEAR_SHOULDER_R) == NULL)
		{
	      ...
	      send_to_char
		("You already wear two items on your shoulders.\n\r", ch);
	      return;
	    }
	}
      if (CAN_WEAR (obj, ITEM_WEAR_SHIELD))
	{
	  ...
	  obj2 = get_eq_char (ch, WEAR_HOLD_2);
          if (IS_PLAYER (ch) && (is_member(ch, GUILD_THIEFG) && (ch->pcdata->perm_stat[STR_I] < 24) && (obj->pIndexData->weight > 2)))
            {
              act ("Unfortunately, $p is too heavy for you to use!", ch, obj, NULL, TO_CHAR);
              return;
            }
	  if (!is_member(ch, GUILD_THIEFG) && obj->pIndexData->item_type == ITEM_ARMOR
	  ...
	  /*&& hand_empty( ch ) != WEAR_NONE */ )
	{
	  ...
	  obj3 = get_eq_char (ch, WEAR_SHIELD);
           if (obj1 !=NULL && (is_member(ch, GUILD_THIEFG) && ch->pcdata->learned[gsn_dual_daggers] < 50) 
                           && (obj2 != NULL && obj2->pIndexData->item_type != ITEM_WEAPON))
             {
              send_to_char ("You can't wield two daggers without knowing Dual Daggers!\n\r", ch);
              return;
            }
          if (obj1 !=NULL && !is_member(ch, GUILD_THIEFG) && ch->pcdata->learned[gsn_dual_wield] < 50 
                          && (obj2 != NULL && obj2->pIndexData->item_type != ITEM_WEAPON))
             {
              send_to_char ("You can't wield two weapons without knowing Dual Wield!\n\r", ch);
              return;
            }
	  ...
	  if (IS_PLAYER (ch)
	      && (obj->pIndexData->weight > ((get_curr_str (ch) * 2) / 3)))
	    {
	      act ("Unfortunately, $p is too heavy for you to use!", ch, obj, NULL, TO_CHAR);
	      return;
	    }
...
{
  DEFINE_COMMAND ("swap", do_exchange, POSITION_RESTING, 0, LOG_NORMAL,
		  "This command exchanges what you have in your primary and secondary hand.")
    return;
}*/
...
do_exchange (CHAR_DATA * ch, char *argy)
{
  ...
  bool hone;
  bool htwo;
  DEFINE_COMMAND ("exchange", do_exchange, POSITION_FIGHTING, 0, LOG_NORMAL,
		  "This command exchanges what you have in your primary and secondary hand.")
    hone = FALSE;
  htwo = FALSE;
  if ((objh1 = get_eq_char (ch, WEAR_HOLD_1)) != NULL)
    hone = TRUE;
  if ((objh2 = get_eq_char (ch, WEAR_HOLD_2)) != NULL)
    htwo = TRUE;
  if ((!hone) && (!htwo))
    {
      act ("You aren't holding anything in your hands!", ch, NULL, NULL,
	   TO_CHAR);
      return;
    }
  ...
  if ((!hone) && (htwo))
    {
      ...
      act ("The object in your secondary hand is now in your primary hand.",
  ...
  if ((hone) && (!htwo))
    {
      ...
      act ("The object in your primary hand is now in your secondary hand.",
	   ch, NULL, NULL, TO_CHAR);
      return;
    }
  ...
  if ((hone) && (htwo))
    {
      objh1->wear_loc = WEAR_HOLD_2;
      objh2->wear_loc = WEAR_HOLD_1;
      act ("You now hold $p\x1B[0m in your primary hand.", ch, objh2, NULL,
	   TO_CHAR);
      act ("You now hold $p\x1B[0m in your secondary hand.", ch, objh1, NULL,
	   TO_CHAR);
      return;
    }
...
{
  DEFINE_COMMAND ("wield", do_wear, POSITION_STANDING, 0, LOG_NORMAL,
		  "Allows you to wear/wield or hold an object.") return;
}

void
do_hold (CHAR_DATA * ch, char *argy)
{
  DEFINE_COMMAND ("hold", do_wear, POSITION_STANDING, 0, LOG_NORMAL,
		  "Allows you to wear/wield or hold an object.") return;
}*/
...
do_wear (CHAR_DATA * ch, char *argy)
{
  ...
  char arg2[SML_LENGTH];
  SINGLE_OBJECT *obj;
  DEFINE_COMMAND ("wear", do_wear, POSITION_FIGHTING, 0, LOG_NORMAL,
		  "Allows you to wear/wield or hold an object.") argy =
    one_argy (argy, arg1);
  one_argy (argy, arg2);
  if (arg1[0] == '\0')
    {
...
do_sheath (CHAR_DATA * ch, char *argy)
{
  ...
  SINGLE_OBJECT *obj;
  DEFINE_COMMAND ("sheath", do_sheath, POSITION_STANDING, 0, LOG_NORMAL, "Allows you to sheath a weapon in your belt.") argy =
    one_argy (argy, arg1);
/*get_obj_held */
  if ((obj = get_obj_inv (ch, arg1)) == NULL)
    {
...
do_draw (CHAR_DATA * ch, char *argy)
{
  char arg1[STD_LENGTH];
  SINGLE_OBJECT *obj;
  DEFINE_COMMAND ("draw", do_draw, POSITION_FIGHTING, 0, LOG_NORMAL,
		  "Allows you to draw a weapon from your belt.")
    if (get_item_held (ch, ITEM_WEAPON) != NULL
	&& get_obj_wear (ch, "shield") != NULL)
    {
...
do_remove (CHAR_DATA * ch, char *argy)
{
  char arg[SML_LENGTH];
  SINGLE_OBJECT *obj;
  DEFINE_COMMAND ("remove", do_remove, POSITION_FIGHTING, 0, LOG_NORMAL,
		  "This command allows you to remove a weapon/armor/etc... that is worn.")
    one_argy (argy, arg);
  if (arg[0] == '\0')
    {
...
{
  DEFINE_COMMAND ("bury", do_sacrifice, POSITION_RESTING, 0, LOG_NORMAL,
		  "Allows you to sacrifice something to the gods... helps clean up the game.")
    return;
}*/
...
do_sacrifice (CHAR_DATA * ch, char *argy)
{
  ...
  DEFINE_COMMAND ("sacrifice", do_sacrifice, POSITION_RESTING, 0, LOG_NORMAL,
		  "Allows you to sacrifice something to the gods... helps clean up the game.")
    if (argy == "" || argy[0] == '\0')
    return;
  one_argy (argy, arg);
  ...
  if (obj == NULL)
	...
	{
	  send_to_char
	    ("The object must be lying on the ground in order to sacrifice it.\n\r",
	     ch);
	  return;
	}
  ...
  if (ch->pcdata->no_quit_pk < 5 && get_curr_int(ch) > 19 && get_curr_wis(ch) > 19
      && obj->pIndexData->item_type == ITEM_CORPSE_NPC 
      && is_member (ch, GUILD_WIZARD) || is_member (ch, GUILD_HEALER) || is_member (ch, GUILD_ELEMENTAL) || is_member (ch, GUILD_NECROMANCER))
  {
    int count;
...
do_quaff (CHAR_DATA * ch, char *argy)
{
  ...
  SINGLE_OBJECT *obj;
  I_POTION *pt;
  DEFINE_COMMAND ("quaff", do_quaff, POSITION_FIGHTING, 0, LOG_NORMAL,
		  "Allows you to quaff a potion.") one_argy (argy, arg);
  if (arg[0] == '\0')
    {
...
{
  DEFINE_COMMAND ("read", do_look, POSITION_RESTING, 0, LOG_NORMAL,
		  "Same as look.") return;
}
...
do_recite (CHAR_DATA * ch, char *argy)
{
  ...
  SINGLE_OBJECT *obj;
  DEFINE_COMMAND ("recite", do_recite, POSITION_FIGHTING, 0, LOG_NORMAL,
		  "Allows you to recite a scroll.") argy =
    one_argy (argy, arg1);
  argy = one_argy (argy, arg2);
...
do_brandish (CHAR_DATA * ch, char *argy)
{
  ...
  SPELL_DATA *spell;
  I_WAND *stf;
  DEFINE_COMMAND ("brandish", do_brandish, POSITION_FIGHTING, 0, LOG_NORMAL,
		  "Allows you to brandish a staff.") if ((staff =
							  get_item_held (ch,
									 ITEM_STAFF))
							 == NULL)
    {
  ...
  if (stf->current_charges > 0)
    {
      act
	("$n raises $p\x1B[0m into the air and utters some mystical words under $s breath.",
  ...
  if (--stf->current_charges <= 0)
    {
      act ("$n's $p\x1B[0m blazes bright and is gone.", ch, staff, NULL,
	   TO_ROOM);
      act ("Your $p\x1B[0m blazes bright and is gone.", ch, staff, NULL,
	   TO_CHAR);
      free_it (staff);
    }
...
do_zap (CHAR_DATA * ch, char *argy)
{
  ...
  SPELL_DATA *spell;
  I_WAND *wnd;
  DEFINE_COMMAND ("zap", do_zap, POSITION_FIGHTING, 0, LOG_NORMAL,
		  "Allows you to zap a wand.") one_argy (argy, arg);
  ...
  if (--wnd->current_charges <= 0)
    {
      act ("$n's $p\x1B[0m explodes into fragments.", ch, wand, NULL,
	   TO_ROOM);
      act ("Your $p\x1B[0m explodes into fragments.", ch, wand, NULL,
	   TO_CHAR);
      free_it (wand);
    }
...
do_study (CHAR_DATA * ch, char *argy)
{
  ...
  SPELL_DATA *spell;
  I_BOOK *bk;
  DEFINE_COMMAND ("study", do_study, POSITION_RESTING, 0, LOG_NORMAL,
		  "Allows you to study a book.") if ((obj =
						      get_obj_inv (ch,
								   argy)) ==
						     NULL)
    {
  ...
  if (obj->pIndexData->item_type != ITEM_BOOK)
    {
      send_to_char
	("That's not a book.. wouldn't want it to BURST into FLAMES! heh.\n\r",
	 ch);
      return;
    }
  bk = (I_BOOK *) obj->more;
  ...
  if (spell == NULL)
    {
      send_to_char
	("Some LAME builder created this book wrong - it doesn't do SHIT!\n\rHAHAHAHAHA!",
	 ch);
      return;
    }
  if (pow.professions && !(ch->pcdata->profession_flag & spell->values[0]))
    {
      send_to_char
	("You can't seem to understand the spell it's talking about...\n\r",
	 ch);
      return;
    }
/*past this point the book FRIES.. */
  ...
  act ("$n reads $p\x1B[0m, and it bursts into flames!", ch, obj, NULL,
       TO_ROOM);
  act ("After you read $p\x1B[0m, it bursts into flames!", ch, obj, NULL,
       TO_CHAR);
  free_it (obj);
  return;
}
...
do_steal (CHAR_DATA * ch, char *argy)
{
  ...
  if (IS_PLAYER (ch) && (ch->in_battle || ch->challenge == 10))
    {
  ...
  if (is_safe (ch, victim))
	...
	{
	  act
	    ("\x1B[1;37mYou feel as if the gods are watching, and a hot pain spreads",
	     ch, NULL, NULL, TO_CHAR);
	  act
	    ("through your hand. OUCH! Perhaps you should not steal here...\x1B[0m",
	     ch, NULL, NULL, TO_CHAR);
	}
      return;
    }
  ...
  else
    {
	  ...
	  || (IS_PLAYER (ch)
	      && number_range (0, 115) > ch->pcdata->learned[gsn_steal])
	  || (IS_MOB (ch) && (number_range (1, 10) < 4)) || (IS_MOB (victim)
							     && victim->
							     pIndexData->
							     pShop != NULL))
	{
	  ...
	  if (IS_PLAYER (ch))
	      ...
	      else
		{
		  ...
		  if (!IS_EVIL (ch))
		    send_to_char
		      ("You are well on your way to becoming a wanted criminal!\n\r",
		       ch);
		}
	    }
	  return;
      ...
      else
	{
	  ...
	  if (obj->wear_loc != -1)
	    {
	      send_to_char
		("You can't steal things people are WEARING or HOLDING!!!\n\r",
		 ch);
	      return;
	    }
	  if (!can_drop_obj (ch, obj)
	  ...
	  if (number_range (1, 5) == 2)
	    {
	      act
		("$N's suddenly looks your way, and you whistle innocently.",
...
do_light (CHAR_DATA * ch, char *argy)
{
  ...
  I_LIGHT *lt;
  I_TOOL *tb;
  DEFINE_COMMAND ("light", do_light, POSITION_RESTING, 0, LOG_NORMAL,
		  "Allows you to light a light source.") argy =
    one_argy (argy, arg);
  if (arg[0] == '\0')
  ...
  if (use_tool (obj2, TOOL_TINDERBOX))
	...
	{
	  send_to_char
	    ("Your tinderbox is almost out of tinder, and the flint is starting to chip.\n\r",
	     ch);
	}
      if (tb->uses == 0)
	{
	  send_to_char
	    ("Your tinderbox is out of tinder and the flint is chipped...\n\rTime to buy a new one!\n\r",
	     ch);
	  obj_from (obj2);
	  free_it (obj2);
	}
...
do_extinguish (CHAR_DATA * ch, char *argy)
{
  ...
  SINGLE_OBJECT *obj;
  I_LIGHT *lt;
  DEFINE_COMMAND ("extinguish", do_extinguish, POSITION_RESTING, 0,
		  LOG_NORMAL,
		  "Allows you to extinguish a light source.") argy =
    one_argy (argy, arg);
  if (arg[0] == '\0')
    obj = get_item_held (ch, ITEM_LIGHT);
/*held */
...
do_butcher (CHAR_DATA * ch, char *argy)	/*By Arcane */
{
  ...
  int count, x;
  char buf[80];
  DEFINE_COMMAND ("butcher", do_butcher, POSITION_STANDING, 0, LOG_NORMAL,
		  "Allows you to butcher a corpse into steaks.  Based on your butcher skill.")
    if (argy[0] == '\0')
    {
